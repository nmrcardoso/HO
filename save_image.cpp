...
Image::Image(const char *f, DrawPlot **wgt, int n) : _file(NULL), _buffer(NULL), _error(0) {
	if(wgt){
		int w = 0, h = 0;
		for(int i = 0; i < n; ++i){
			w += wgt[i]->w();
		...
		if (_file == NULL) { return; }
		_buffer = new(std::nothrow) uchar[3 * w * h];
		uchar *_buffer1 = new(std::nothrow) uchar[3 * w * h];
		if (_buffer == NULL) { return; }
		int w0 = 0;
		for(int i = 0; i < n; ++i){
			fl_read_image(_buffer1, wgt[i]->x(), wgt[i]->y(), wgt[i]->w(), wgt[i]->h());
			for(int j = 0; j < wgt[i]->h(); ++j){
				memcpy(_buffer + j * 3 * w + i * w0 * 3, _buffer1 + 3 * wgt[i]->w() * j, 3 * wgt[i]->w());
			}
			w0 += wgt[i]->w();
		}
		delete[] _buffer1;	
