...
void DrawTicks::calculate(double xmin, double xmax, double ymin, double ymax){
	if(NxDivLvl1) xticks.CalculateTicks(ScreenAxisLength_x, NxDivLvl1, NxDivLvl2, xmin, xmax);
	if(NyDivLvl1) yticks.CalculateTicks(ScreenAxisLength_y, NyDivLvl1, NyDivLvl2, ymin, ymax);
}



void DrawTicks::drawAxis(){
...
void Ticks::CalculateTicksValues(int nbins, double funcMin_, double funcMax_){
	if(TicksPosLvl1){ delete[] TicksPosLvl1; TicksPosLvl1 = 0; NumTicksLvl1 = 0; }
	if(TicksValuesLvl1){ delete[] TicksValuesLvl1; TicksValuesLvl1 = 0; NumTicksLvl1 = 0; }
	if(TicksPosLvl2){ delete[] TicksPosLvl2; TicksPosLvl2 = 0; NumTicksLvl2 = 0; }

	int nDivOpt;
	double step1=0, step2=0, fmin2=0, fmax2=0;
	double fmin = funcMin_;
	double fmax = funcMax_;
	int fNDiv = nbins;
	int fNDivLvl1 = fNDiv%100;

	// Level 1 tick marks.
	OptimizeTicks(fmin,  fmax, fNDivLvl1, funcMin_, funcMax_, nDivOpt, step1);
	fNDivLvl1   = nDivOpt;
	NumTicksLvl1 = fNDivLvl1+1;
	TicksValuesLvl1  = new double[NumTicksLvl1];
	for(int i=0; i < NumTicksLvl1; ++i){
		TicksValuesLvl1[i] = funcMin_ + i*step1;
		//cout << TicksValuesLvl1[i] << endl;
		//if( abs(TicksValuesLvl1[i]) < 1.e-10) std::cout << (fmax-fmin)/abs(TicksValuesLvl1[i]) << std::endl;
		if( abs(TicksValuesLvl1[i]) < 1.e-10 && abs((fmax-fmin)/TicksValuesLvl1[i]) > 1.e10 ) TicksValuesLvl1[i] = 0.;
	}	
}
